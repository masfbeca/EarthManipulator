...
SkyNode::SkyNode(const std::string& starFile, float minStarMagnitude ) : 
_minStarMagnitude(minStarMagnitude)
{
    initialize(starFile);
}

SkyNode::SkyNode( float minStarMagnitude) : 
_minStarMagnitude(minStarMagnitude)
{
    initialize();
}
...
void
SkyNode::initialize(const std::string& starFile )
{
    ...
    _outerRadius = _innerRadius * 1.030f;
...
{    
    ...
    if ( _ellipsoidModel.valid() )
    {
        ...
        {
            std::cout << "You must provide an EphemerisProvider" << std::endl;
        }
...
SkyNode::makeAtmosphere( const osg::EllipsoidModel* em )
{
    ...
    set->setAttributeAndModes( new osg::BlendFunc( GL_ONE, GL_ONE ), osg::StateAttribute::ON );
		...
		string str = s_versionString;
		str+=s_mathUtils;
		str+=s_atmosphereVertexDeclarations;
		str+=s_atmosphereVertexShared;
		str+=s_atmosphereVertexMain;

        osg::Shader* vs = new osg::Shader( osg::Shader::VERTEX, str);
        program->addShader( vs );

		string str2 = s_versionString;

		str2+=s_mathUtils;

		str2+=s_atmosphereFragmentDeclarations;
		str2+=s_atmosphereFragmentMain;
        osg::Shader* fs = new osg::Shader( osg::Shader::FRAGMENT, str2);
...
SkyNode::makeSun()
{
    ...
    float sunRadius = _innerRadius * 200.0f;
   ...
   // set->setAttributeAndModes( new osg::BlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA), osg::StateAttribute::ON );
    ...
    // create shaders
        ...
        osg::Program* program = new osg::Program();
		string str3 = s_versionString;
		str3+=s_sunVertexSource;
        osg::Shader* vs = new osg::Shader( osg::Shader::VERTEX, str3);
        program->addShader( vs );

		string str4 = s_versionString;
		str4+="precision highp float;\n";
		str4+=s_mathUtils;
		str4+=s_sunFragmentSource;
        osg::Shader* fs = new osg::Shader( osg::Shader::FRAGMENT, str4);
        program->addShader( fs );
        set->setAttributeAndModes( program, osg::StateAttribute::ON );
...
SkyNode::makeMoon()
{
    ...
    moon->getOrCreateStateSet()->setAttributeAndModes( new osg::Program(), osg::StateAttribute::ON | osg::StateAttribute::PROTECTED );
    ...
    if ( 1 )
    {
        ...
        // create shaders
		string str5 = s_versionString;
		str5+="precision highp float;\n";
		str5+=s_moonVertexSource;
        osg::Program* program = new osg::Program();
        osg::Shader* vs = new osg::Shader( osg::Shader::VERTEX, str5);
        program->addShader( vs );
		string str6 = s_versionString;
		str6+="precision highp float;\n";
		str6+=s_moonFragmentSource;
        osg::Shader* fs = new osg::Shader( osg::Shader::FRAGMENT, str6 );
        program->addShader( fs );
...
SkyNode::makeStars(const std::string& starFile)
{
  ...
  {
    if( !starFile.empty() )
      std::cout << "Warning: Unable to use star field defined in file \"" << starFile << "\", using default star data." << std::endl;
...
SkyNode::parseStarFile(const std::string& starFile, std::vector<StarData>& out_stars)
{
  ...
  {
    std::cout
		<<  "Warning: Unable to open file star file \"" << starFile << "\"" << std::endl;
    return false ;
  }
