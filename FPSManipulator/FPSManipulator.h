...
class __declspec(dllexport) FPSManipulator : public osgGA::CameraManipulator
{
public:

	explicit FPSManipulator(void);
	explicit FPSManipulator(const FPSManipulator &param, const osg::CopyOp &copyop = osg::CopyOp::SHALLOW_COPY);

	void initParameters();

	virtual ~FPSManipulator();

    virtual const char* className() const { return "FPSManipulator"; }
