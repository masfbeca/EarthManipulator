...
static double getHeightOfDriver()
{
	...
	std::cout<<"FPSManipulator::_height set to =="<<height<<std::endl;
	return height;
}

FPSManipulator::FPSManipulator(const FPSManipulator &param, const osg::CopyOp &copyop)
{
	initParameters();
}
...
void FPSManipulator::initParameters() 
{
...
void FPSManipulator::setNode(osg::Node* node)
{
	_node = node;
	if (_node.get())
	{
...
const osg::Node* FPSManipulator::getNode() const
{
	return _node.get();
}


osg::Node* FPSManipulator::getNode()
{
	return _node.get();
}

bool FPSManipulator::intersect(const osg::Vec3d& start, const osg::Vec3d& end, osg::Vec3d& intersection, osg::Vec3d& normal) const
{
...
void FPSManipulator::computeHomePosition()
{
	if(_node.get())
	{
		const osg::BoundingSphere& boundingSphere=_node->getBound();
...
void FPSManipulator::home(double )
{
	...
	case(GUIEventAdapter::KEYDOWN):
		{
			...
			else if (ea.getKey()==GUIEventAdapter::KEY_Shift_L 
				    || ea.getKey()==GUIEventAdapter::KEY_Shift_R)
			{
	...
	case(GUIEventAdapter::KEYUP):
		{
			//	flushKBEventStack();
			//	W A S D
			if (ea.getKey() == ea.getKey() == 'R' || ea.getKey() == 'r')
			{
				_pitchWKeyPressed = false;
			}
			else if (ea.getKey() == ea.getKey() == 'D' || ea.getKey() == 'd')
			{
				_pitchAKeyPressed = false;
			}
			else if (ea.getKey() == 'F' || ea.getKey() == 'f')
...
void FPSManipulator::getUsage(osg::ApplicationUsage& usage) const
{
...
void FPSManipulator::flushKBEventStack()
{
	_ga_kb_t0 = NULL;
	_ga_kb_t1 = NULL;
}
...
void FPSManipulator::addKBEvent(const GUIEventAdapter& ea)
{
	_ga_kb_t1 = _ga_kb_t0;
	_ga_kb_t0 = &ea;
}
void FPSManipulator::addMouseEvent(const GUIEventAdapter& ea)
{
	_ga_t1 = _ga_t0;
	_ga_t0 = &ea;
	_isKBControl=false;
...
void FPSManipulator::computePosition(const osg::Vec3 &vecEye, const osg::Vec3 &vecCenter, const osg::Vec3 &vecUp)
{
...
#pragma region 移动速度合成
	...
	//	加上速度
	{
		...
		//A D键 左右 左右移动速度是前后移动速度的1/2，更接近现实
...
osg::Vec3 FPSManipulator::getFinalPosition(osg::Vec3 oldPosition, osg::Vec3 speed)
{
	osg::Matrixd rotation_matrix;
	rotation_matrix.makeRotate(_rotation);
...
osg::Vec3 FPSManipulator::getPosition(int recursionDepth, osg::Vec3 oldPosition, osg::Vec3 speed)
{
...
void FPSManipulator::resetCamera(void)
{	
