...
#define LAST_ANGLE 0.07
#define OLD_VERSION 0
#define THROWN_TIME 0.09
#define FURTHEST_DISTANCE 60000000
...
_lock_azim_while_panning        ( false ),
_mouse_sens                     ( 1.0 ),
_keyboard_sens                  ( 1.0 ),
_scroll_sens                    ( 1.0 ),
_min_pitch                      ( -89.9 ),
...
EarthManipulator::applySettings( Settings* settings )
{
    ...
    if ( new_pitch != old_pitch )
    {
        ...
        setViewpoint( Viewpoint(vp.getFocalPoint(), vp.getHeading(), new_pitch, vp.getRange()));
    }
}
...
EarthManipulator::established()
{
  ...
  //      {
		...
		//	//_csn->addChild(_node.get());
		//	_csn->setEllipsoidModel(pEllipsoidModel.release());
		//}
		...
		if (_csn.valid() )
		{
            ...
            if ( !_homeViewpoint.isSet() )
            {
                ...
                {
					//setHomeViewpoint( 
					//	Viewpoint(osg::Vec3d(113.6,36,0), 0, -89,
					//	_csn->getEllipsoidModel()->getRadiusEquator()*2.6 ) );
					setHomeViewpoint( 
						Viewpoint(osg::Vec3d(113.6,36,0), 0, -89,
						FURTHEST_DISTANCE ) );
                }
...
//                 setViewpoint( _pending_viewpoint, _pending_viewpoint_duration_s );
	...
	bool csnValid = _csn.valid();
	bool nodeValid = _node.valid();
    return csnValid && nodeValid;
...
EarthManipulator::createLocalCoordFrame( const osg::Vec3d& worldPos, osg::CoordinateFrame& out_frame ) const
{
	...
	osg::Vec3d mapPos;
	{
        ...
        _csn->getEllipsoidModel()->computeLocalToWorldTransformFromLatLongHeight(
...
{
	bool bNodeValid = _node.valid();
    if ( node == 0L || !bNodeValid )
    {
        _node = node;
        _csn = 0L;
...
EarthManipulator::setViewpoint( const Viewpoint& vp, double duration_s )
{
    ...
    else if ( duration_s > 0.0 )
    {
        ...
        if ( _is_geocentric )
        {
            ...
            _csn->getEllipsoidModel()->convertLatLongHeightToXYZ(
        ...
        {
			double maxDistance = _csn->getEllipsoidModel()->getRadiusEquator();
	...
	_zoomFactor = getZoomFromulaXFactorFromDistance();
}
...
void EarthManipulator::handleThrown()
{
	float t = _lastReleaseTime-_lastDragTime;
	if (t<THROWN_TIME)
	{
		_lastAngle*=0.989;	
		if (fabs(_lastAngle)<FLT_EPSILON)
		{
			_lastAngle=0;
		}
		osg::Quat q;
		q.makeRotate(_lastAngle,_lastNormal);
...
EarthManipulator::handle(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa)
{
	...
	case osgGA::GUIEventAdapter::RELEASE:
		_lastReleaseTime = osg::Timer::instance()->time_s();
		if ( _continuous )
		{
	...
	case osgGA::GUIEventAdapter::DRAG:
		{
			_lastDragTime = osg::Timer::instance()->time_s();
...
EarthManipulator::serviceTask()
{
    if ( _task.valid() && _task->_type != TASK_NONE )
    {
        ...
        {
            case TASK_PAN:
				handleKeyboardPan(_task->_dx, _task->_dy );
...
void EarthManipulator::handleKeyboardPan(double dx, double dy)
{
	osg::Vec3d axis;
	double angle;
...
float EarthManipulator::tb_project_to_sphere( float r, float x, float y )
{
	float d, t, z;
...
void
EarthManipulator::handlePan(double dx, double dy ,osg::View* view)
{
	osg::Vec3d worldStartDrag;
	screenToWorld(_ga_t0->getX(), _ga_t0->getY(),view, worldStartDrag);

	if (worldStartDrag.length2()==0.0)
	{
		getPoint(_ga_t0->getXnormalized(), _ga_t0->getYnormalized(),worldStartDrag, view);
	}

	osg::Vec3d worldEndDrag;
	screenToWorld(_ga_t1->getX(), _ga_t1->getY(),view, worldEndDrag);

	if (worldEndDrag.length2()==0.0)
	{
		getPoint(_ga_t1->getXnormalized(), _ga_t1->getYnormalized(),worldEndDrag, view);
	}

	osg::Vec3d offset = worldStartDrag-worldEndDrag;
	const osg::Vec3d zeroPoint = osg::Vec3d(0.0,0.0,0.0);
	
	osg::Vec3d startNormal = worldStartDrag-zeroPoint;
	osg::Vec3d endNormal = worldEndDrag-zeroPoint;

	startNormal.normalize();
	endNormal.normalize();

	osg::Vec3d axisNormal = startNormal^endNormal;
	double sinTheta = axisNormal.length();
	axisNormal.normalize();

	osg::Quat q;
	q.makeRotate(sinTheta,axisNormal);
	_lastAngle = sinTheta;
	if (_lastAngle>LAST_ANGLE)
	{
		_lastAngle = LAST_ANGLE;
	}
	
	_lastNormal=axisNormal;
	_center = q*_center;
	_centerRotation=_centerRotation*q;
	setCenter(_center);
}
...
double EarthManipulator::getDistanceFromZoomFormula()
{
	return pow(0.8,_zoomFactor-50);
}
double EarthManipulator::getZoomFromulaXFactorFromDistance()
{
	return (log10(_distance/1000)/log10(0.8))+50;
}
void
EarthManipulator::zoom( double dx, double dy)
{    
	double offset = (_zoomSensitivity*dy);
	_zoomFactor+=offset;
	//计算出鼠标滚动时鼠标所在位置
	osg::Vec3d mousePosition;
	osg::Vec3d focalPoint = _center;
	screenToWorld(_ga_t0->getX(), _ga_t0->getY(),_zoomView, mousePosition);
	//根据focalPoint和mousePosition计算出旋转角度和旋转轴
	osg::Vec3d axis = mousePosition^focalPoint;
	double cosTheta = (mousePosition*focalPoint)/(mousePosition.length()*focalPoint.length());
	double theta = acos(cosTheta);
	//计算出旋转角度比例
	double oldDistance = _distance;	
	double tempDistance = getDistanceFromZoomFormula();
	//当距地面距离小于一个常数时，就不向前继续移动相机了，可以切换漫游器。
	if (tempDistance<=15||tempDistance>=(FURTHEST_DISTANCE/1000))
	{
		_zoomFactor-=(_zoomSensitivity*dy);
		return;
	}
	
	
	_distance = tempDistance;
	double ratio = (oldDistance-_distance*1000)/oldDistance;
	double rotateAngle = ratio*theta;
	if (fabs(rotateAngle)<FLT_EPSILON)
	{
		rotateAngle=0;
	}
	//进行旋转
	...
	osg::Quat _temp;
	_temp.makeRotate(-rotateAngle,axis);
	_center = _temp*_center;
	_centerRotation=_centerRotation*_temp;
	setCenter(_center);
	//panWithZoom(_distance * scale);
}
        ...
        switch( action._type )
        {
            case ACTION_GOTO:
            {
				...
				_csn->getEllipsoidModel()->convertXYZToLatLongHeight(
					...
					pointVP.y(), pointVP.x(), pointVP.z() );

				pointVP.y() = osg::RadiansToDegrees(pointVP.y());
				pointVP.x() = osg::RadiansToDegrees(pointVP.x());
				...
				if (here.getRange()>=FURTHEST_DISTANCE)
				{
					here.setRange(FURTHEST_DISTANCE);
				}
...
EarthManipulator::handleKeyboardAction( const Action& action, double duration )
{
    double dx = 0, dy = 0;
	//键盘旋转速度随距离变化而变化
	double offsetFactor = 0.000000417;
	//除1000是将米转换为千米
	float offset = offsetFactor*_distance/1000;
	cout<<offset<<endl;
    switch( action._dir )
    {	
		case DIR_LEFT:  dx =  offset; break;
		case DIR_RIGHT: dx = -offset; break;
		case DIR_UP:    dy = -offset; break;
		case DIR_DOWN:  dy =  offset; break;
		default: break;
    }
...
EarthManipulator::handleAction( const Action& action, double dx, double dy, double duration )
{
    ...
    case ACTION_HOME:
        ...
        {
			_thrown=false;
			float times = action.getDoubleOption(OPTION_DURATION, 1.0);
            setViewpoint( _homeViewpoint.value(), times*3 );
        }
        break;
...
void EarthManipulator::getPoint(double x, double y,osg::Vec3d& out_coords,osg::View* theView)
{
	osgViewer::View* view = dynamic_cast<osgViewer::View*>( theView );
	if ( !view || !view->getCamera() )
		return ;
	...
	const osg::Vec2 ptMouseOrg(x, y);
	const Radial3	rayOrg = getCameraScreenRadial(view->getCamera(), ptMouseOrg);
	osg::Plane planeBase(rayOrg.getDirection(), osg::Vec3(0,0,0));
	osg::Vec3		ptRotationOrg;
	getHitTestPoint(rayOrg,planeBase,ptRotationOrg);

	//osg::Vec3d startVertex = osg::Vec3d(local_x,local_y,(zNear+zFar)/2) * inverse;
	osg::Vec3d startVertex = ptRotationOrg;
	osg::Vec3d endVertex = osg::Vec3d(0.0,0.0,0.0);
...
bool EarthManipulator::getHitTestPoint(const Radial3 &ray, const osg::Plane &plane, osg::Vec3 &vecRet, bool bIngoreNegtive)
{
	const float fltCosTheta = plane.getNormal() * ray.getDirection();
	if(fabs(fltCosTheta) <= FLT_EPSILON)
	{
		// 射线和平面基本平行，不予求交
		vecRet.set(0.0f, 0.0f, 0.0f);
		return false;
	}

	const osg::Vec3::value_type fltRatio = ray.IntersectPlane(plane);
	...
	if(bIngoreNegtive && fltRatio < FLT_EPSILON)
	{
	...
	vecRet = ray.getPoint(fabs(fltRatio));
	return true;
}


Radial3 EarthManipulator::getCameraScreenRadial(const osg::Camera *pCamera, const osg::Vec2 &ptPosNormalize)
{
	...
	osg::Vec3 vecRayDir = ptRayEnd - ptRayBegin;
	const osg::Vec3::value_type fltLen = vecRayDir.normalize();
	if(fltLen <= FLT_EPSILON)
	{
		return Radial3();
	}

	return Radial3(ptRayBegin, vecRayDir);
}
#if 0
void  EarthManipulator::getCameraPose(CameraPose &pose) const
{
	osg::Matrixd inverseMat = getInverseMatrix();
	pose = Matrixd_2_CameraPose(inverseMat);
}

void  EarthManipulator::setCameraPose(const CameraPose &pose) 
{
	osg::notify(osg::NOTICE)
	    << "AzimuthAngle = " <<pose.m_dblAzimuthAngle<< std::endl;
	osg::Matrixd viewMat = CameraPose_2_Matrixd(pose);
	setByInverseMatrix(viewMat);
}

INavigatorManager *createNavigatorManager(void)
{
	osg::ref_ptr<EarthManipulator> pEarthManipulator = new EarthManipulator();
	return pEarthManipulator.release();
}
#endif
